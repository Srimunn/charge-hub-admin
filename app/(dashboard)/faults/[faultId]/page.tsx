...
export default function FaultDetailPage() {
  ...
  useEffect(() => {
    const fetchFault = async () => {
      try {
        ...
        console.error(err);
        toast({ title: "Error", description: "Failed to load fault details", variant: "destructive" });
      } finally {
        setIsLoading(false);
      }
    };
    if (faultId) fetchFault();
  }, [faultId, toast]);

  if (isLoading) {
    return <div className="p-8 text-center">Loading fault details...</div>;
  }
  ...
  const handleRetry = () => {
    toast({ title: 'Retrying Session', description: `Attempting to resume session for fault ${fault._id}` });
  };

  const handleLockStation = () => {
    toast({ title: 'Station Locked', description: `Station ${fault.stationId?.name} has been locked for inspection` });
  };

  const handleResolve = async () => {
    try {
      await resolveFault(fault._id);
      setFault({ ...fault, status: 'resolved', resolvedAt: new Date() });
      toast({ title: 'Fault Resolved', description: `Fault ${fault._id} marked as resolved` });
    } catch (err: any) {
  ...
  const handleCreateTicket = () => {
    toast({ title: 'Ticket Created', description: `Maintenance ticket created for fault ${fault._id}` });
  };
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <div className="flex-1 p-6 space-y-6">
        <Button variant="ghost" onClick={() => router.push('/faults')} className="gap-2">
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="p-4 rounded-lg bg-secondary/50">
                ...
                <p className="font-medium">{format(new Date(fault.timestamp), 'MMM dd, HH:mm:ss')}</p>
              </div>
            </div>
